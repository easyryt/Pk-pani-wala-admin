...
const ChargesList = () => {
  ...
  const fetchCharges = async () => {
    ...
    try {
      const response = await fetch("https://pkpaniwala.onrender.com/admin/FloorWiseCharges/getAll", {
        headers: {
          "x-admin-token": token,
  ...
  const handleDeleteCharge = async (chargeId) => {
    const token = Cookies.get("token");
    try {
      const response = await fetch(`https://pkpaniwala.onrender.com/admin/FloorWiseCharges/delete/${chargeId}`, {
        method: "DELETE",
        headers: {
          "x-admin-token": token,
          "Content-Type": "application/json",
