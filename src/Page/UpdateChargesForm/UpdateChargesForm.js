...
const UpdateChargesForm = () => {
  ...
  useEffect(() => {
    const fetchChargeDetails = async () => {
      ...
      try {
        const response = await fetch(`https://pkpaniwala.onrender.com/admin/FloorWiseCharges/getAll?isBulk=${status}`, {
          headers: {
            "x-admin-token": token,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://pkpaniwala.onrender.com/admin/FloorWiseCharges/update/${status}`, {
        method: "PUT",
        headers: {
          "x-admin-token": token,
          "Content-Type": "application/json",
