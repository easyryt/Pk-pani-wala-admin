...
import { Box, Grid, TextField, Button, Card, CardContent, Typography, CardMedia, FormControl, InputLabel, Select, MenuItem, CircularProgress, IconButton, Dialog, DialogActions, DialogContent, DialogTitle, Snackbar } from "@mui/material";
...
const ProductsPage = () => {
  ...
  useEffect(() => {
    axios
      .get("https://pkpaniwala.onrender.com/public/product/products?bulkAvailable=true&bulkAvailable=false")
      .then((response) => {
  ...
  const filterProducts = (searchQuery, bulkFilter) => {
    ...
    if (bulkFilter) {
      filtered = filtered.filter((product) => product.bulkAvailable === (bulkFilter === "true"));
    }
  ...
  const confirmDelete = () => {
    ...
    if (selectedProduct) {
      axios
        .delete(`https://pkpaniwala.onrender.com/admin/product/delete/${selectedProduct}`, {
          headers: {
            "x-admin-token": token,
          },
        })
      ...
      {isLoading ? (
        ...
        <Grid container spacing={3}>
          {filteredProducts.length === 0 ? (
            <Typography variant="h6" sx={{ width: "100%", textAlign: "center", marginTop: 5 }}>
              No products found
            </Typography>
          ) : (
              ...
              <Grid item xs={12} sm={6} md={4} key={product._id}>
                <Card sx={{ borderRadius: "12px", boxShadow: "0 4px 12px rgba(0, 0, 0, 0.1)" }}>
                  ...
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ marginTop: 1 }}>
                      {product.bulkAvailable ? `Min Quantity: ${product.bulkMinQuantity}` : "Single purchase"}
                    </Typography>
